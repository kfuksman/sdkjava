...
public class PostRequest extends Request {
     ...
     * @param url
     */
    public PostRequest(String token, String account, String url) {
    ...
    public PostRequest execute() {
        ...
        try {
            constructUrl();
            if (body != null){
                RequestBodyEntity request = Unirest.post(urlBuilder.toString())
                        .header("x-auth-token", token).body(body);
                responseApi = request.asString();
            } else {
                MultipartBody request = Unirest.post(urlBuilder.toString())
